...
Purpose: Execute Turnstile/anti-fraud fingerprint validation through a real browser to avoid account being flagged by internal risk control
(which results in successful registration but subsequent Team invitation features being disabled).
...
Flow:
  ...
  7. Return to chatgpt.com → retrieve access_token from /api/auth/session
  8. Retrieve session_token / oai-did from Cookie
...
def browser_register(cfg, mail_provider) -> dict:
    """Walk through the registration process with a real browser.
    cfg: Config instance (requires proxy field)
    mail_provider: MailProvider instance (calls create_mailbox + wait_for_otp)
    Returns dict: compatible with AuthResult.to_dict() format"""
    from camoufox.sync_api import Camoufox
    from browserforge.fingerprints import Screen

    email = mail_provider.create_mailbox()
    # Preferentially reuse same-origin persona generated by mail_provider algorithm (email prefix consistent with first/last + password=local reversed)
    ...
    if persona is not None:
        ...
        # Compatible with resume / old path: remove @ from email as password + choose name independently
    ...
    try:
        with Camoufox(
            ...
            # [1] Open the ChatGPT homepage, click "Sign up for free"
            logger.info("[browser-reg] 打开 ChatGPT 首页 ...")
            page.goto("https://chatgpt.com/", wait_until="domcontentloaded", timeout=60000)
            # Wait for React rendering to complete + Sign up button is interactive
            try:
            ...
            # Click the Sign up button — find "Sign up for free" in the upper right corner
                ...
                for btn in btns:
                    try:
                        ...
                        if "sign up" not in text:
                            continue
                        # Click with 5s timeout to prevent hanging for 30s
                        try:
                            btn.click(timeout=5000)
                        except Exception:
                            # If click gets stuck, trigger with JS
            ...
            # Wait for redirect to auth.openai.com or modal loading (including retry clicks)
            pre_url = page.url
            for i in range(20):
                time.sleep(1)
                if "auth.openai.com" in page.url or page.query_selector('input[type="email"]'):
                    break
                # If there is no change after 5s, retry clicking Sign up
            ...
            # [2a] New OpenAI (from 2026-05): After clicking Sign up, does not redirect to auth.openai.com,
            # Instead, a "Log in or sign up" modal pops up on chatgpt.com, inside which is
            # Continue with Google / Apple / Phone + OR + Continue with email。
            # The old script would directly wait for input[type=email] and timeout after 30s, so first identify the modal,
            # Turn off Google One-Tap, then click "Continue with email".
            try:
                # Google One-Tap iframe (with "Sign in with Google" in the title) will overlay on top of the modal
                # Turn off first to prevent intercepting clicks.
                for ot_sel in [
                    ...
                    try:
                        ...
                        if f and f.is_visible():
                            if "iframe" in ot_sel:
                                # iframe cannot be clicked directly, delete the container with JS directly
                                page.evaluate(
            ...
            if not page.query_selector('input[type="email"], input[name="email"]'):
                # If the email input field is not currently visible, find the email entry button in the modal and click it again.
                # Order: First exact match "Continue with email", then loosen to buttons containing email.
                ...
                for sel in [
                    ...
                    for b in btns:
                        try:
                            ...
                            # Exclude social buttons that happen to contain "email" in Google/Apple/Phone
            ...
            # [2] Fill in email (click + fill in separate steps, React re-rendering may cause handle to become invalid → re-query for each step)
            ...
            # [3] Fill in password (new accounts will see the password field)
            ...
            # [4] Turnstile / hCaptcha wait (Camoufox fingerprint can usually pass automatically)
            ...
            if page.query_selector('input[autocomplete="one-time-code"]') or \
               page.query_selector('input[inputmode="numeric"]'):
                ...
                # Fill OTP
                otp_filled = False
                # Might be single-frame / multi-frame two types
                ...
                # OpenAI displays "Incorrect code" in red text when OTP is wrong, and allows repeated attempts
                # Continue will trigger max_check_attempts risk control (permanently freeze). Early exit.
                try:
            ...
            # [6] /about-you: Full name + Age (single input field)
            logger.info(f"[browser-reg] OTP 后 URL: {page.url[:120]}")
            time.sleep(5)  # Redirect equal weight to /about-you
            logger.info(f"[browser-reg] 稳定后 URL: {page.url[:120]}")

            # Wait for /about-you form to load completely. First wait for URL to stabilize
            for _ in range(20):
                time.sleep(1)
                if "about-you" in page.url or "chatgpt.com" in page.url:
                    break

            # OpenAI about-you variant:
            #   Old version: Full name + Age (numeric input field)
            #   New version (from 2026-04): Full name + Birthday (date field, pre-filled with today)
            # Export metadata of all inputs at once using JS to avoid visibility detection inconsistencies
            def _enum_inputs():
                try:
            ...
            def _is_name_input(meta: dict) -> bool:
                ...
                # Old version about-you used "age" numeric field; new version uses "Full name" + "Birthday"
                return any(kw in blob for kw in ("name", "first", "last", "full",
                                                  "given", "family", "age"))

            def _looks_like_chat_ui() -> bool:
                """chatgpt.com homepage characteristics: chat input box in bottom-right corner + "New chat" on sidebar.
                This page is not an about-you form, seeing 2 inputs doesn't mean filling them blindly."""
                try:
            ...
            for attempt in range(30):
                ...
                # First prioritize Birthday + keyword-matched name input — both sides' keywords must match to count.
                ...
                if bd and name_m:
                    ...
                    break
                # Compatible with old age version: 2 inputs + at least one matches name keyword + URL not in
                # chatgpt.com main chat page (avoid treating chat textarea + search as a form to fill blindly).
                if (
                    not bd
                    ...
                    break
                # Already on chatgpt.com homepage (not in about-you subpath), and cannot see about-you form
                # —— registration may have completed directly, break the loop and let outer layer judge accessToken.
                if (
            ...
            if full_name_input and birthday_input:
                ...
                # Birthday: January 15 between ages 26-40 (old enough >18, fixed date for consistent fingerprinting)
                import datetime as _dt
                year = _dt.datetime.now().year - random.randint(26, 40)
                mm, dd = "01", "15"
                # Native date input uses YYYY-MM-DD, text boxes mostly use MM/DD/YYYY
                ...
                try:
                    ...
                    # Clear first (pre-fill may have today's date)
                    try:
                    ...
                    if bd_type == "date":
                        ...
                        # MM/DD/YYYY: for compatibility with old age version, if it looks like number/age just type age
            ...
            for i in range(120):
                ...
                if cur != last_url:
                    ...
                    last_url = cur
                # At chatgpt.com and React main interface has loaded
                if "chatgpt.com" in cur and "auth.openai.com" not in cur:
                    # Wait for /api/auth/session to normally return accessToken to count as complete
                    try:
                        ...
                        pass
                # If still at auth.openai.com, may have /email-verification or other redirects, continue clicking continue
                if "auth.openai.com" in cur and i % 10 == 5:
                    for sel in ['button:has-text("Continue")', 'button:has-text("Next")',
                        ...
                        try:
                            ...
                            # Context destroyed during page navigation, ignore
                            pass
            if not arrived:
            ...
            # [10] Codex OAuth to obtain refresh_token
            # Known limitation: after signup completes, hydra session on auth.openai.com cannot exchange token for Codex
            # (login_session is only signup challenge state, not a complete user session)
            # Current refresh_token will be empty; if refresh_token is needed, need to login account and re-go through Codex OAuth
            #
            # Empirically verified (2026-04 recent daemon + self-dealer full logs), signup-state Codex OAuth
            # 100% returns token_exchange_user_error, wasting ~30s each time. Skip by default; if old path needs to be retained
            # As reverse reference, set SKIP_SIGNUP_CODEX_RT=0. Subsequently _exchange_refresh_token_with_session
            ...
            if str(os.environ.get("SKIP_SIGNUP_CODEX_RT", "1")).lower() in ("1", "true", "yes", "on"):
                ...
                try:
                    ...
                    auth_params = {
                        ...
                        # Without prompt=none: session already established through browser signup,
                        # let server auto-recognize session, auto-approve consent page when present
                    }
