...
async function handlePairingMode() {
  if (!PAIRING_PHONE) {
    writeState({ status: 'error', error: 'WA_PAIRING_PHONE 未设置（带国家码无加号，例如 REDACTED_PHONE）' });
    log('pairing mode requires WA_PAIRING_PHONE env (digits only, e.g. REDACTED_PHONE)');
    return;
  }
